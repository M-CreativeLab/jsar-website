...
#### 静态方法 `void DispatchHandRay(int, Ray)`

用户分发射线事件，在 JSAR 内部，我们使用射线来实现物体和 UI 的交互，你可以将你的射线信息通过该方法分发给 JSAR，参数说明如下：

- `int handType`：手势类型，0 为左手，1 为右手
- `Ray handRay`：射线信息，包含射线的起点和方向
...
由于 JSAR 是动态加载物体，因此我们在集成到 Unity 应用中，需要手动包含一些 Shaders，以保证 JSAR 小程序的正常渲染：
