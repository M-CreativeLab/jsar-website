# 创建项目

JSAR 可以通过多种方式初始化项目，包括：

- 通过 Npm 命令本地创建
- 通过 GitHub Template 创建为 GitHub 项目

## 通过 Npm 创建

开发者可以通过 Npm 创建，但需要先安装 [Node.js](https://nodejs.org/en/)，安装完成后，打开终端，执行如下命令：

```bash
$ npm init @yodaos-jsar/widget
```

该命令会按照下面的步骤初始化项目：

- 下载最新的模版项目 [M-CreativeLab/template-for-jsar-widget](https://github.com/M-CreativeLab/template-for-jsar-widget)
- 根据你的输入更新 package.json 中的项目名称

运行成功后，会获得一个 package.json，可以按照如下编写：

```json
{
  "name": "jsar-gallery-rokid-jungle",
  "displayName": "Rokid Jungle",
  "version": "1.0.0",
  "description": "The mstudio gallery for JSAR",
  "main": "main.xsml",
  "files": [
    "icon.png",
    "main.xsml",
    "lib/*.ts",
    "model/foobar.glb"
  ],
  "icon3d": {
    "base": "./model/foobar.glb"
  },
  "author": "your name and email",
  "license": "MIT",
  "devDependencies": {
    "@yodaos-jsar/types": "^1.4.0"
  }
}
```

初始化成功后，为了让项目正常运行，还需要安装依赖：

```bash
$ npm install
```

它用于安装 JSAR 空间组件的类型定义文件，这样可以在开发过程中获得类型检查和自动补全的功能。

## 通过 GitHub Template 创建

> 如果你希望通过本地创建项目，请忽略这一小节。

打开 [M-CreativeLab/template-for-jsar-widget](https://github.com/M-CreativeLab/template-for-jsar-widget) 后，点击 `Use this template` 按钮，创建一个新的 GitHub 项目。

![use this template](./howto-use-github-template-01.png)

点击 `Create repository from template` 按钮，将开始创建一个新的 GitHub 项目，或者也可以直接[点击这里](https://github.com/new?template_name=template-for-jsar-widget&template_owner=M-CreativeLab)。

![use this template](./howto-use-github-template-02.png)

按照上图所示，填写相关信息即可，这样创建好的项目就是一个 JSAR 空间组件项目，它同样包含我们模版项目中的文件，以下是一些通过 GitHub Template 创建的项目：

- https://github.com/M-CreativeLab/jsar-gallery-solar-system
- https://github.com/M-CreativeLab/jsar-gallery-flatten-lion

> 如果通过 GitHub 项目创建的 JSAR 空间组件，推荐为你的项目添加 "jsar-widget" 的主题（Topic），这样可以方便 JSAR 开发团队以及社区在 [GitHub #jsar-widget](https://github.com/topics/jsar-widget) 上发现你的项目。

## package.json 字段说明

- `name`：项目名称，必须是小写字母，可以包含连字符和下划线，不能包含空格。
- `displayName`：项目显示名称，可以包含中文。
- `version`：项目版本号，遵循 [语义化版本](https://semver.org/lang/zh-CN/) 规范。
- `description`：项目描述。
- `main`：项目入口文件，必须是 xsml 文件。
- `files`：项目文件列表，包含 xsml 文件、icon 图片、模型文件等，只有添加到 `files` 字段中的文件或文件夹才会被打包到 JSAR 空间组件包中。
- `icon3d`：项目 3D 图标配置，包含 `base` 和 `size` 两个字段，其中 `base` 是模型文件路径，`size` 是模型文件的尺寸，单位是米。
- `author`：项目作者，可以是姓名和邮箱。
- `license`：项目许可证，遵循 [SPDX](https://spdx.org/licenses/) 规范。
- `devDependencies`：项目开发依赖，包含 `@yodaos-jsar/types`，这是 JSAR 空间组件的类型定义文件。

特别注意事项：

1. `name` 字段必须是小写字母，不支持 scoped namespace，例如 `@rokid/jsar-gallery-rokid-jungle` 是不支持的。
2. `main` 字段必须是 xsml 文件，不支持 js 文件。
3. `files` 字段必须包含 xsml 文件和`icon.png`，后者用于展示在 JSAR 空间组件商店及小组件列表中。
4. 请务必添加 `@yodaos-jsar/types` 这个依赖，它包含了 JSAR 空间组件的类型定义文件，这样可以在开发过程中获得类型检查和自动补全的功能。

## 使用 TypeScript 智能提示

为了使用 JSAR 的 TypeScript 的智能提示，除了安装 `@yodaos-jsar/types` 之外，还需要在项目根目录下创建 `tsconfig.json` 文件，内容如下：

```json
{
  "compilerOptions": {
    "module": "commonjs",
    "target": "es6",
    "types": [
      "@yodaos-jsar/types"
    ]
  },
  "exclude": [
    "node_modules"
  ]
}
```
